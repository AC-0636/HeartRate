...
public class HeartRateActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private Camera.PreviewCallback mPreviewCallback = new Camera.PreviewCallback() {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            ...
            int imgAvg = ImageProcessing.decodeYUV420SPtoRedAvg(data.clone(), height, width); //OBTAIN
            Log.e("HeartRateActivity","onPreviewFrame");
            heartRateChart.lineTo(imgAvg);
            if (imgAvg == 0 || imgAvg == 255 || imgAvg < 150) {
                ToastUtil.showToast(HeartRateActivity.this,"Please cover the camera with your finger.");
                reStart();
            ...
            {
                Rdata=processData(averageData);
                int bpm=Rdata*6;
                String temp= null;
                temp=Integer.toString(bpm);
               Intent Result = new Intent(HeartRateActivity.this,HeartBeatResult.class);
               Result.putExtra("data",temp);
    ...
    private SurfaceHolder.Callback mSurfaceCallback = new SurfaceHolder.Callback() {
        ...
        public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
            ...
            //   parameters.setFlashMode(Camera.Parameters.FLASH_MODE_TORCH);//Turn on flash light
